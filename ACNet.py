...
class ACNet:
    def __init__(self, scope, a_size, trainer,TRAINING,GRID_SIZE,GLOBAL_NET_SCOPE):
        ...
        if TRAINING:
            ...
            self.imitation_loss = tf.reduce_mean(tf.keras.backend.categorical_crossentropy(self.optimal_actions_onehot,self.policy)) 
            ...
            self.var_norms     = tf.compat.v1.global_norm(local_vars)
            ...
            self.i_var_norms     = tf.compat.v1.global_norm(local_vars)
    ...
    def _build_net(self,inputs,goal_pos,RNN_SIZE,TRAINING,a_size):
        ...
        policy_layer = slim.fully_connected(inputs=self.rnn_out, num_outputs=a_size,weights_initializer=normalized_columns_initializer(1./float(a_size)), biases_initializer=None, activation_fn=None)
